...
`--map` автоматично розпаковує стиснуті пакети (zstd/kanzi) — окремий прапор
не потрібен. Щоб export віддавав стиснутий TDTP, додайте в `output.tdtp` пайплайну:

```yaml
output:
  tdtp:
    destination: "out/bulk_{{limit}}.tdtp.xml"
    compression: true
    compress_algo: zstd      # або kanzi (на ~30% щільніше)
    compress_level: 3        # zstd 1-19, kanzi 6-7
```

Реальні цифри (1478 осіб): zstd lvl 3 стискає **218 KB → 52 KB (−76%)**.
Локально `--map` стиснутого файлу на ~19% повільніший (декомпресія CPU) —

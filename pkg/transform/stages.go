...
// ── Чтение ──────────────────────────────────────────────────────────────────
//
// Порядок на чтении НЕ является обращением порядка записи, и это не мелочь:
// три из четырёх багов вокруг колоночной раскладки случились именно здесь, а
// диспетчер записи от них не защищает.
//
// Записывающий порядок отвечает на вопрос «что во что заворачивать». Читающий
// отвечает на другой — «что уже можно трогать». Расхождение берётся оттуда,
// что часть проверок применима лишь к развёрнутым данным, а часть обязана
// произойти раньше, чем данные развернут.
//
// ReadOrder перечисляет шаги чтения. Порядок внутри списка и есть ответ.
var readOrder = []ReadStep{
	{
		Name: "parse",
		Note: "разбор XML; здесь же отвергается схема-переросток — до того, как " +
			"под неё выделят память",
	},
	{
		Name: "decrypt",
		Note: "расшифровка секций: пока они непрозрачны, ни развернуть, ни " +
			"сосчитать нечего",
	},
	{
		Name: "decompress",
		Note: "распаковка. ЗДЕСЬ детонирует бомба разжатия, и никакой хеш от неё " +
			"не спасает: подпись проверяется позже, по распакованным строкам. " +
			"Ограничение MaxDecompressedBytes — единственное, что стоит между " +
			"25 КБ пакета и 200 МБ в памяти",
	},
	{
		Name: "expand-columnar",
		Note: "разворот колонок в строки. ОБЯЗАН идти до счёта строк и до " +
			"проверки целостности: распакованное — это по строке на КОЛОНКУ, и " +
			"восемь колонок против десяти строк заголовка ломали --to-csv, " +
			"--to-html, --to-tdtp и --import одинаково",
	},
	{
		Name: "expand-compact",
		Note: "разворот carry-forward: значения фиксированных полей " +
			"восстанавливаются из предыдущих строк",
	},
	{
		Name: "verify-rowcount",
		Note: "сверка RecordsInPart. После разворотов, иначе считаются колонки " +
			"или сжатые чанки. Хешем счётчик НЕ покрыт — computeHashes берёт " +
			"Schema и значения строк, заголовок в них не входит",
	},
	{
		Name: "verify-integrity",
		Note: "xxh3 по построчным значениям — тем самым, что хешировал писатель " +
			"до сжатия. Отсюда и место: раньше разворотов хеш считался бы по " +
			"другим байтам, чем у producer'а",
	},
	{
		Name: "expand-dictionary",
		Note: "подстановка полных значений вместо коротких токенов; на хеши не " +
			"влияет, потому что идёт после проверки",
	},
}

// ReadStep — шаг чтения и причина его места в порядке.
type ReadStep struct {
	Name string
	Note string
}

// ReadOrder возвращает порядок шагов чтения.
//
// Это описание, а не исполнитель: шаги разбросаны по ParseBytes,
// DecompressData, VerifyIntegrity и командам CLI. Список существует затем,
// чтобы место каждого было записано вместе с причиной — три бага здесь
// случились ровно потому, что причина жила в чьей-то голове.
func ReadOrder() []ReadStep {
	out := make([]ReadStep, len(readOrder))
	copy(out, readOrder)
	return out
}

// ReadStepIndex возвращает позицию шага чтения, или -1.
func ReadStepIndex(name string) int {
	for i, s := range readOrder {
		if s.Name == name {
			return i
		}
	}
...
// ReadMustPrecede сообщает, обязан ли a идти раньше b при чтении.
func ReadMustPrecede(a, b string) bool {
	ia, ib := ReadStepIndex(a), ReadStepIndex(b)

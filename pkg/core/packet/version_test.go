...
// BumpVersion поднимает версию до минимума, но никогда не опускает:
// версия пакета — это максимум использованных возможностей.
func TestBumpVersion(t *testing.T) {
	cases := []struct {
		have, min, want string
	}{
		{"1.0", "1.2", "1.2"},     // сжатие на свежем пакете
		{"1.0", "1.3.1", "1.3.1"}, // компакт на свежем пакете
		{"1.2", "1.4", "1.4"},     // целостность поверх сжатия
		{"1.4", "1.2", "1.4"},     // сжатие поверх целостности — не ронять
		{"1.3.1", "1.2", "1.3.1"}, // сжатие поверх компакта — не ронять
		{"1.5", "1.4", "1.5"},     // шифрование старше всего
		{"1.4", "1.4", "1.4"},     // равные — без изменений
		{"1.10", "1.4", "1.10"},   // числовое сравнение, не строковое
		{"abc", "1.2", "abc"},     // мусор — не трогать (считается новее)
		{"", "1.2", ""},           // пустая — не трогать
		{"1.0", "abc", "1.0"},     // мусорный минимум — не трогать
	}
	for _, c := range cases {
		pkt := NewDataPacket(TypeReference, "t")
		pkt.Version = c.have
		BumpVersion(pkt, c.min)
		if pkt.Version != c.want {
			t.Errorf("BumpVersion(%q, %q) = %q, want %q", c.have, c.min, pkt.Version, c.want)
		}
	}
}

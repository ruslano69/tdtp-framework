...
func (sg *StreamingGenerator) createPart(
	...
	// SpecialValues детектируются НА ЧАСТЬ, а не на всю выгрузку.
	//
	// Без этого поток отдавал сырой nullSentinel — байт 0x00 — там, где обычный
	// экспорт пишет канонический маркер [NULL]. То есть выбор режима менял
	// значения: на наборе с 10% NULL расходились 10 026 строк из 100 000.
	// GenerateReference зовёт DetectAndApply один раз по всем строкам, а в
	// потоке всех строк нет по определению — к моменту записи первой части
	// последняя ещё не прочитана.
	//
	// Подетально это корректно, потому что Schema и так копируется в каждую
	// часть: часть самодостаточна и описывает ровно то, что в ней лежит.
	// Импорту это не мешает — SchemaEquals сверяет имена и типы полей, а не
	// набор спецзначений.

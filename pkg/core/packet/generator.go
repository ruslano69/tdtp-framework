...
func estimateRowSize(row []string) int {
	...
	for _, value := range row {
		size += writtenValueLen(value) + 1 // +1 для разделителя
	}
	size += 10      // XML теги <R></R>
	return size * 2 // UTF-16 для MSMQ
}

// writtenValueLen — длина значения в том виде, в каком оно попадёт в пакет.
//
// Отличается от len() ровно в одном случае: NULL приходит из адаптера одним
// байтом NUL, а записывается как "[NULL]" — шесть. Разница видна только там,
// где размер считают ДО простановки маркеров, то есть в потоковом экспорте:
// обычный путь зовёт DetectAndApply один раз по всем строкам и меряет уже
// готовые значения, а поток меряет строку на входе и проставляет маркеры позже,
// в createPart.
//
// Из-за этого потоковые части выходили крупнее заказанных, и промах рос вместе
// с долей пустых ячеек: на таблице с одной колонкой из десяти, пустой в 10%
// строк, это 0.3%, а на восьми полностью пустых датовых колонках — 43%, то есть
// 2.78 МБ там, где просили 2. Флаг --packet-size существует ровно для случая,
// когда размер ограничен снаружи, так что промах в эту сторону обесценивает
// его.
//
// Остальные маркеры считать не нужно, и это не небрежность: они значение
// СОКРАЩАЮТ ("Infinity" → "INF") либо не меняют длины ("NaN" → "NaN",
// "0001-01-01" → "0000-00-00"). Недоучёт даёт часть больше заказанной, перебор
// — меньше; ошибаться безопасно только во вторую сторону.
func writtenValueLen(value string) int {
	if value == nullSentinel {
		return len(SpecNullMarker)
	}
	return len(value)
}

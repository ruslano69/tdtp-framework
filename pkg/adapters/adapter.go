...
type Config struct {
	...
	// StrictSchema — при CreateTable восстанавливать заявленную длину
	// текстовых полей (VARCHAR(n)/CHAR(n)) вместо безусловного TEXT.
	// По умолчанию false: TEXT без ограничений, чтобы импорт не падал на
	// обрезании (см. историю в pkg/adapters/postgres/types.go — этот же
	// откат уже случился один раз). При true колонка получает то же
	// ограничение длины, что было на источнике, и вставка строки длиннее
	// n падает с ошибкой БД сразу, вместо молчаливого расширения до TEXT —
	// то есть обменивает удобство на "падать вовремя, а не после".
	//
	// Точное совпадение длины гарантировано только когда пакет
	// импортируется в ту же СУБД, из которой был экспортирован: единица
	// длины (символы/байты) и правила усечения различаются между
	// движками. Сейчас читается только адаптером PostgreSQL.
	StrictSchema bool
}

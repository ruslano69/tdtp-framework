...
func (a *Adapter) GetTableSchema(tableName string) (packet.Schema, error) {
	...
	rows, err := a.pool.Query(a.ctx, query, a.schema, tableName)
	if err != nil {
	...
	pkColumns, err := a.getPrimaryKeyColumns(tableName)
	if err != nil {
...
func (a *Adapter) getPrimaryKeyColumns(tableName string) ([]string, error) {
	query := `
		SELECT a.attname
		FROM pg_index i
...
func (a *Adapter) ExportTableWithQuery(tableName string, query *packet.Query, sender, recipient string) ([]*packet.DataPacket, error) {
	...
	allPackets, err := a.ExportTable(tableName)
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) readRowsWithSQL(sql string, schema packet.Schema) ([][]string, error) {
	rows, err := a.pool.Query(a.ctx, sql)
	if err != nil {
...
func (a *Adapter) createQueryContextForSQL(query *packet.Query, rows [][]string, tableName string) *packet.QueryContext {
	...
	a.pool.QueryRow(a.ctx, countSQL).Scan(&totalRecords)

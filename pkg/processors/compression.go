...
// Алгоритмы сжатия, поддерживаемые TDTP.
const (
	AlgoZstd  = "zstd"
	AlgoKanzi = "kanzi"
)
...
func DecompressDataForTdtp(compressed string) ([]string, error) {
	return DecompressDataForTdtpWithAlgo(compressed, AlgoZstd)
}
...
func DecompressDataForTdtpWithAlgo(compressed, algo string) ([]string, error) {
	...
	switch algo {
	case AlgoKanzi:
		decompressedData, err = DecompressKanzi([]byte(compressed))
	default: // AlgoZstd и всё остальное
...
// --- Dispatcher: выбор алгоритма по имени ---

// CompressDataForTdtpAlgo сжимает строки TDTP-пакета выбранным алгоритмом.
func CompressDataForTdtpAlgo(rows []string, algo string, level int) (compressedRow string, stats CompressionStats, err error) {
	if len(rows) == 0 {
		return "", CompressionStats{}, nil
	}
	...
	switch algo {
	case AlgoKanzi:
		compressedData, err = CompressKanzi(originalData, level)
	default: // AlgoZstd
		compressedData, err = Compress(originalData, level)
	}
	if err != nil {
...
//
// ВЫЗЫВАЮЩИХ НЕТ. Держится как публичный API для внешних потребителей.
//
// Прежний комментарий гласил «используется в --test: содержимое <Data>
// непрозрачно, RecordsInPart — авторитетный счётчик». Обе половины устарели:
// --test с 1f31620 считает строки сам, а счётчик из заголовка авторитетным не
// является — он не покрыт ни одним хешем, и его сверяет VerifyRowCount. Строка
// осталась бы инструкцией для следующего читателя завести баг заново.

...
COLUMNAR_COMPRESSED_SAMPLE_FILE = TESTDATA / "users_columnar_compressed.tdtp.xml"
COMPACT_SAMPLE_FILE      = TESTDATA / "dept_emp_compact.tdtp.xml"
...
# Generated by: create_test_db.py → tdtpcli --export Users --compress
COMPRESSED_TOTAL_ROWS    = 10
COMPRESSED_TABLE_NAME    = "Users"

# users_columnar_compressed.tdtp.xml: the exact same 10 rows as
# users_compressed.tdtp.xml, re-exported with --columnar --compress (Data
# layout="columns", zstd). Same row count and table name — the fixture exists
# to prove the two read paths agree, not to add new data.
COLUMNAR_COMPRESSED_TOTAL_ROWS = COMPRESSED_TOTAL_ROWS
COLUMNAR_COMPRESSED_TABLE_NAME = COMPRESSED_TABLE_NAME

# Known compact fixture properties (kept in sync with dept_emp_compact.tdtp.xml)
# 3 departments × employees: Sales(3), IT(2), HR(1) = 6 rows total
COMPACT_TOTAL_ROWS        = 6
COMPACT_TABLE_NAME        = "dept_emp"
COMPACT_FIELD_NAMES       = ["DeptID", "DeptName", "Location", "EmpID", "Name", "Salary"]
COMPACT_SALES_COUNT       = 3   # rows where DeptName = 'Sales'
COMPACT_IT_COUNT          = 2   # rows where DeptName = 'IT'
COMPACT_HR_COUNT          = 1   # rows where DeptName = 'HR'
COMPACT_SALARY_GT_60000   = 2   # rows where Salary > 60000 (Dave 70k, Eve 75k)
COMPACT_SALARY_GE_60000   = 3   # rows where Salary >= 60000 (Carol 60k, Dave 70k, Eve 75k)
...
@pytest.fixture(scope="session")
def columnar_compressed_tdtp_path() -> Path:
    """Path to the columnar+zstd fixture (Data layout="columns", generated
    from users_compressed.tdtp.xml's own rows — see conftest.py comment)."""
    if not COLUMNAR_COMPRESSED_SAMPLE_FILE.exists():
        pytest.skip(f"Columnar compressed fixture not found: {COLUMNAR_COMPRESSED_SAMPLE_FILE}")
    return COLUMNAR_COMPRESSED_SAMPLE_FILE


@pytest.fixture(scope="session")
def compact_tdtp_path() -> Path:
    """Path to the compact v1.3.1 fixture (dept × employees with carry-forward fixed fields)."""
    if not COMPACT_SAMPLE_FILE.exists():
        pytest.skip(f"Compact fixture not found: {COMPACT_SAMPLE_FILE}")
    return COMPACT_SAMPLE_FILE


@pytest.fixture(scope="session")
def compact_data_j(j_client: TDTPClientJSON, compact_tdtp_path: Path) -> dict:
    """Pre-parsed compact fixture via JSON API — fixed fields must be expanded."""
    return j_client.J_read(str(compact_tdtp_path))

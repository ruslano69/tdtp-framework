...
func ConvertTDTPToHTML(opts HTMLOptions) error {
	...
	// Generate HTML
	htmlContent, renderedRows, err := renderHTML(opts.InputFile, pkt, opts)
	if err != nil {
		return err
	}
...
// Returns the HTML string, the number of rows actually rendered, and any error.
func renderHTML(inputFile string, pkt *packet.DataPacket, opts HTMLOptions) (string, int, error) {
	...
	if opts.Query != nil {
		...
		execResult, err := executor.Execute(opts.Query, allRows, pkt.Schema)
		if err != nil {
			return "", 0, fmt.Errorf("failed to apply query filters: %w", err)
		}
		allRows = execResult.FilteredRows
		totalRows = len(allRows)
	}
...
`)
  ...
  <span><strong>%d</strong> primary key(s)</span>
	...
	return b.String(), len(parsedRows), nil
}

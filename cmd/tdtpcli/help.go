...
func PrintHelp() {
	...
	fmt.Println()
	fmt.Println("  tdtpcli supports database views for export operations:")
	fmt.Println()
	fmt.Println("  ✅ Export: Works with views (if you know the view name)")
	fmt.Println("  ❌ List: --list shows only BASE TABLEs, not views")
	fmt.Println("  ⚠️  Import: Works only with updatable views")
	fmt.Println()
	fmt.Println("  # Export from view (specify view name explicitly)")
	fmt.Println("  tdtpcli --export my_view --output view_data.xml")
	fmt.Println()
	fmt.Println("  # Export from view with filters")
	fmt.Println("  tdtpcli --export sales_summary_view --where 'year = 2024' --output sales.xml")
	fmt.Println()
	fmt.Println("  # To find view names, use database-specific tools:")
	fmt.Println("  # PostgreSQL: SELECT table_name FROM information_schema.views WHERE table_schema='public';")
	fmt.Println("  # SQLite:     SELECT name FROM sqlite_master WHERE type='view';")
	fmt.Println("  # MS SQL:     SELECT name FROM sys.views;")
	fmt.Println()

...
Опционально: логин по паролю → короткоживущий токен, обязательный для
всего `/api/*` (кроме `/api/login` самого). Без секции `auth:` в конфиге
(или `auth.enabled: false`) `/api/*` остаётся полностью открытым — как
было до появления авторизации. HTML (`/`, `/data/`) авторизацией не
покрывается: отдельное решение по месту.
...
auth:
  enabled: true
  token_ttl: 30m          # необязательно, по умолчанию 30m
  users:
    - username: hr_app
      password_hash: "$2a$10$..."   # см. ниже, никогда не plaintext
```

### Генерация `password_hash`

```bash
tdtpserve --hash-password
Password: ****           # ввод скрыт, не попадает в историю shell
Confirm:  ****
$2a$10$9WB6m1prSYxb4M4ikCEeiuTa2Y4p88Pn26gIVSsAIXwUWAYYe5aW.
```

Полученную строку — целиком, включая `$2a$10$...` — вставить в
`password_hash`. Требует реального терминала (скрытый ввод пароля не
работает через pipe/non-interactive stdin).

### Использование

```
POST /api/login {"username": "hr_app", "password": "..."}
  → 200 {"token": "...", "expires_at": "2026-07-23T08:19:29+03:00"}
  → 401 при неверном логине/пароле (без уточнения, какое из двух —
    чтобы нельзя было перебором выяснить существующие username)

GET /api/data/Users
  Authorization: Bearer <token>
  → 200 как обычно
  → 401 без заголовка, с невалидным или просроченным токеном

POST /api/logout
  Authorization: Bearer <token>
  → 200 — токен отозван немедленно, не дожидаясь TTL
  → 401 если токен уже невалиден
```

Логин ограничен **10 попытками в минуту с одного адреса**; сверх — `429` с
заголовком `Retry-After`. Лимит считает все попытки, не только неудачные:
он про нагрузку (каждый вызов запускает bcrypt), а не про правильность
пароля.

### Ключевые решения

- **bcrypt для пароля, не SHA-256** — пароль угадываемый (человеческий),
  нужен намеренно медленный соленый хеш; SHA-256 годится только для уже
  высокоэнтропийных токенов (как в `cmd/orchestrator/auth.go`).
- **Непрозрачный токен в памяти, не JWT** — без новой тяжёлой зависимости,
  мгновенный revoke, потеря токенов при рестарте сервера приемлема для
  внутреннего инструмента (перелогин — дешёвая операция).
- **Лимит на `/api/login`** — единственный маршрут, доступный без токена, и
  каждый его вызов стоит ~60 мс CPU на bcrypt. Без лимита это одновременно
  перебор пароля и отказ в обслуживании; один предел закрывает оба.
- Полный дизайн и обоснование — [AUTH_PLAN.md](AUTH_PLAN.md).

### Чего здесь нет

Осознанные ограничения — знать до выноса за пределы доверенной сети:

- **Нет TLS.** Токен идёт в заголовке открытым текстом; кто видит трафик,
  видит и токен. Ставить за reverse-proxy с HTTPS.
- **`X-Forwarded-For` не учитывается** — заголовок подделывается, доверять
  ему значит отдать сброс собственного счётчика в руки клиенту. За прокси
  все запросы делят его адрес, и лимит становится общим: как граница
  нагрузки это верно, как «на клиента» — грубее.
- **Токены живут в памяти** — рестарт разлогинивает всех. Потолок — 10000
  живых токенов, сверх него логин отвечает ошибкой, а не растёт дальше.
- **Нет ролей.** Любой валидный токен видит все `/api/*` целиком. Разделение
  контуров (кадровый / финансовый) сегодня делается отдельными процессами со
  своими конфигами, а не правами внутри одного.

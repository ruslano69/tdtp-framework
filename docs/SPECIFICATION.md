...
**Версия:** 1.4 (базовый протокол v1.0; расширения: v1.2 — compression, v1.3 — encryption, v1.3.1 — compact format / fixed fields / special values, v1.4 — PipelineContext)
**Дата:** 08.05.2026
...
7. [PipelineContext v1.4](#pipelinecontext-v14)
8. [Примеры](#примеры)
...
│
├── PipelineContext     (опциональный)  🆕 v1.4
│   ├── Pipeline        (name, version — имя и версия YAML-конфига)
│   └── Variables[]     (Var: name, value — переменные использованные при экспорте)
│
...
### PipelineContext 🆕 v1.4

Метаданные ETL pipeline, встроенные в пакет при экспорте через `--pipeline`. Позволяет получателю проверить параметры источника до начала импорта.
...
| Элемент / Атрибут | Обязательное | Описание |
|-------------------|--------------|----------|
| `Pipeline @name` | ✅ | Имя pipeline (поле `name:` из YAML-конфига) |
| `Pipeline @version` | ⚪ | Версия pipeline (поле `version:` из YAML-конфига) |
| `Variables/Var @name` | ⚪ | Имя переменной (`@name=value` из CLI) |
| `Variables/Var @value` | ⚪ | Значение переменной на момент экспорта |

**Правила формирования Variables:**

В блок `<Variables>` включаются **только те переменные, которые фактически используются в конфиге** — т.е. встречаются как `@name` в SQL-запросах или `{{name}}` в YAML-полях. Переменные, переданные в CLI но не задействованные в конфиге, исключаются.

**Позиция в пакете:**

`<PipelineContext>` располагается после `<QueryContext>` и до `<Schema>`. Элемент опциональный (`omitempty`) — пакеты без `<PipelineContext>` полностью совместимы с v1.4.

**Поведение при многочастных экспортах:**

Каждая часть многочастного пакета (`TotalParts > 1`) получает идентичный `<PipelineContext>`. Получатель проверяет только первую часть.

**Precondition check — `--expect-var`:**

Получатель может потребовать наличие конкретного значения переменной перед импортом:

```bash
tdtpcli --config cfg.yaml --import data.tdtp.xml --expect-var dept=sales
tdtpcli --config cfg.yaml --import-broker --expect-var dept=sales --expect-var region=EMEA
```

Проверка выполняется **до** создания соединения с БД — импорт прерывается без побочных эффектов:
...
Если пакет не содержит `<PipelineContext>` (создан до v1.4), а `--expect-var` задан:

```
--expect-var: packet has no PipelineContext (was it exported without --pipeline?)
```
...
Раздел содержит полное описание назначения и семантики `<PipelineContext>`.
Структура XML и поля описаны в [Формат пакетов → PipelineContext](#pipelinecontext--v14).

### Зачем нужен PipelineContext

В распределённых пайплайнах (Airline → Central, Branch → Central и т.д.) один и тот же
физический TDTP-поток может содержать данные, экспортированные с разными параметрами:
разный регион, период, отдел. Без метаданных источника получатель не может:

- убедиться, что импортирует данные именно для своего узла;
- обнаружить попадание пакета «не в ту очередь»;
- зафиксировать параметры источника в аудит-логе.

`<PipelineContext>` решает это без дополнительного транспорта — метаданные едут вместе с данными.

### Жизненный цикл

```
Exporter (--pipeline)                     Consumer (--import / --import-broker)
─────────────────────────────             ──────────────────────────────────────
1. ApplyVariables(config, vars)           1. Получить пакет
2. UsedVariables → отфильтровать          2. Проверить PipelineContext:
3. Вставить PipelineContext в             │    --expect-var dept=sales
   каждую часть пакета                   │    --expect-var region=EMEA
4. Сжать / отправить                      3. Fail-fast при несовпадении
                                          4. Импорт в БД
```
...
| Ситуация | Поведение |
|----------|-----------|
| Пакет v1.4 → consumer без `--expect-var` | OK, `<PipelineContext>` игнорируется |
| Пакет v1.4 → consumer с `--expect-var` | Проверка выполняется |
| Пакет v1.0–v1.3.1 → consumer с `--expect-var` | Ошибка: нет PipelineContext |
| Пакет v1.0–v1.3.1 → consumer без `--expect-var` | OK, обратная совместимость |
| `--pipeline` без переменных | PipelineContext с именем pipeline, Variables пустой |
...
### Reference Packet с PipelineContext (v1.4+) 🆕

Пакет, экспортированный через `--pipeline dept_staff.yaml @dept=sales @region=EMEA`:
...
<DataPacket protocol="TDTP" version="1.4">
  <Header>
    <Type>reference</Type>
    <TableName>dept_staff</TableName>
    <MessageID>REF-2026-pc-001-P1</MessageID>
    <PartNumber>1</PartNumber>
    <TotalParts>1</TotalParts>
    <RecordsInPart>3</RecordsInPart>
    <Timestamp>2026-05-08T10:00:00Z</Timestamp>
  </Header>
  <PipelineContext>
    <Pipeline name="dept-sync" version="2.1"/>
    <Variables>
      <Var name="dept"   value="sales"/>
      <Var name="region" value="EMEA"/>
    </Variables>
  ...
  <Schema>
    ...
    <Field name="department" type="TEXT" length="50"></Field>
    <Field name="region"     type="TEXT" length="50"></Field>
    <Field name="hired_at"   type="TIMESTAMP" timezone="UTC"></Field>
  </Schema>
  <Data>
    <R>1|Alice Volkov|sales|EMEA|2024-03-01 09:00:00</R>
    <R>2|Bob Morozov|sales|EMEA|2023-11-15 10:30:00</R>
    <R>3|Carol Sidorova|sales|EMEA|2025-01-20 08:45:00</R>
  </Data>
</DataPacket>
```

Импорт с проверкой источника:

```bash
# OK: переменные совпадают
tdtpcli --config cfg.yaml --import dept_staff.tdtp.xml --expect-var dept=sales --expect-var region=EMEA

# FAIL: получатель ожидал APAC, пришёл EMEA
tdtpcli --config cfg.yaml --import dept_staff.tdtp.xml --expect-var region=APAC
# → --expect-var check failed (pipeline: dept-sync):
# →   @region: expected "APAC", got "EMEA"
```
...
- **v1.4** (08.05.2026) 🆕
  - **PipelineContext** — опциональный XML-элемент `<PipelineContext>` в DataPacket
    - Позиция: после `<QueryContext>`, до `<Schema>`
    - `<Pipeline name="..." version="..."/>` — имя и версия YAML-конфига pipeline
    - `<Variables><Var name="..." value="..."/></Variables>` — переменные, использованные при экспорте
    - Только *использованные* переменные (встречающиеся в SQL/YAML конфига) попадают в пакет
    - Присутствует в каждой части многочастного экспорта
  - **Precondition check `--expect-var name=value`** (CLI)
    - Проверяет `PipelineContext.Variables` до создания соединения с БД
    - Работает для `--import` (файл) и `--import-broker` (очередь)
    - Флаг повторяемый; все условия проверяются одновременно
    - Fail-fast: при несовпадении — ошибка с указанием расхождений, без побочных эффектов
  - **`--inspect` расширен**: печатает `pipeline:` и `pipeline_vars:` если `<PipelineContext>` присутствует
  - **Backward compatibility:** пакеты v1.0–v1.3.1 без `<PipelineContext>` читаются без изменений
  - **Forward compatibility:** consumer без `--expect-var` игнорирует `<PipelineContext>`

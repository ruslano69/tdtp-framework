...
**Рабочая БД — SQLite в памяти, и DuckDB там никогда не было.** `NewWorkspace`
открывает `sqlite` с DSN `:memory:` не глядя в конфиг; поле `type` в
`WorkspaceConfig` есть, но поддерживается только sqlite. `benchmarks/bench_duckdb`
— разовый эксперимент, сравнивавший скорость чтения; в `pkg/` и `cmd/` DuckDB не
упоминается ни разу за всю историю (`git log -S"duckdb" -- pkg/ cmd/tdtpcli`
пуст).

Идея заманчивая: DuckDB колоночный и векторный, а у нас на пайплайне из трёх
источников (2 млн транзакций, соединение с сотрудниками и отделами, группировка
с SUM/AVG/COUNT(DISTINCT)) уходит 18.6 с — 11.0 с загрузка в рабочую БД и ~6.8 с
запрос. Казалось бы, бьёт в обе половины.

**Замерено на живом движке, через условную компиляцию: втрое хуже.**

| 2 млн строк | SQLite | DuckDB |
|---|---|---|
| весь отчёт | **19.0 с** | 61.2 с |
| только загрузка | **11.1 с** | 59.8 с |

Результат при этом идентичен — те же 600 строк отчёта, все тесты `pkg/etl`
проходят на обоих движках без правок общего кода.

Вся разница в загрузке: агрегация у DuckDB как раз быстрее (≈1.3 с против 7.9 с),
но тонет. Причина в том, что общий `LoadData` шлёт многострочные INSERT через
`database/sql`, а у DuckDB **каждый такой вызов пересекает границу cgo**. Его
быстрый путь — Appender (он и используется в `bench_duckdb`, где рядом стоит
комментарий «один CGO-вызов на батч, не на строку»), но Appender не выражается
через `database/sql` и требует отдельного пути загрузки.

**Вывод: без Appender включать DuckDB незачем, а с Appender это уже не «сменить
движок», а написать вторую загрузку.** Плюс цена: cgo, а с ним конец чистой
кросс-компиляции, ради выигрыша на шаге, который в пайплайне не доминирует.

### Диалекты расходятся меньше, чем кажется

Проверено на живом DuckDB, а не по документации. Из того, что реально стоит в
`transform.sql` этого репозитория, **работает без изменений**: `SELECT *`,
`JOIN` + `GROUP BY` с `COUNT/AVG/SUM/MIN/MAX`, `ROUND`, `COALESCE`, `IFNULL`,
`CAST`, `LIMIT`/`OFFSET`, `ORDER BY`, `substr`, `||`, `group_concat`, `rowid`.

Ломается ровно два:

- **`DATE(x)`** — `Catalog Error: Scalar Function with name date does not exist`.
  В DuckDB это тип, а не функция; нужно `CAST(x AS DATE)`. В репозитории три
  таких вызова, все в `09-s3-pipeline-chain`.
- **`strftime`** — аргументы в обратном порядке: `strftime(ts, '%Y-%m')` вместо
  `strftime('%Y-%m', ts)`. Портируемая замена — `substr(d, 1, 7)`.

Настоящая цена не в SQL пользователя, а в типизации: DuckDB строг, а рабочая БД
получает всё строками и полагается на динамическую типизацию SQLite.
`INSERT INTO t VALUES ('не число', ...)` там `Conversion Error`.

### Обход `database/sql` для SQLite — тоже мимо

Раз у DuckDB выигрыш даёт обход обвязки, проверили то же для SQLite: statement у
драйвера напрямую через `Conn.Raw`, аргументы как `[]driver.NamedValue`,
транзакция вручную.

Изолированно работает: на 1 млн строк `LoadData` даёт 2.35 → 2.00 с (−15%), пик
кучи 353 → 289 МБ (−18%), оборот аллокатора вдвое меньше.

**В пайплайне не проявляется.** Загрузка 2 млн: 11.6/11.0/11.1 с против
11.0/11.1/10.9 — в пределах шума; пик процесса скачет 1.1–1.4 ГБ в обе стороны.
Потому что `LoadData` — меньшая часть тех 11 секунд (там ещё чтение источника и
сборка пакета), а пик определяется пакетом в памяти.

Отдельно: **транзакцию при таком замере забыть нельзя.** Первая версия дала
306 мс против 174 именно потому, что у пути через драйвер её не было, и
сравнивалась одна транзакция с десятью тысячами неявных.

...
func (h *hashesHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	// checksum only). Mirrors the framework's NeedsRowCountCheck predicate;
	// duplicated here because xzmercury is a standalone module — see version.go.
	if isLegacyPacketVersion(req.PacketVersion) {
